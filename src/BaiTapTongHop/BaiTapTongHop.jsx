            ...
            <div className='grid grid-cols-6'>
                <div className="col-span-1 h-full p-3 space-y-2 dark:bg-coolGray-900 dark:text-coolGray-100">
                    <div className="flex items-center p-2 space-x-4">
                        <img src="https://source.unsplash.com/100x100/?portrait" alt="" className="w-12 h-12 rounded-full dark:bg-coolGray-500" />
                        <div>
                            <h2 className="text-lg font-semibold">Leroy Jenkins</h2>
                            <span className="flex items-center space-x-1">
                                <a rel="noopener noreferrer" href="#" className="text-xs hover:underline dark:text-coolGray-400">View profile</a>
                            </span>
                        </div>
                    </div>
                    <div className="divide-y divide-coolGray-700">
                        <ul className="pt-2 pb-4 space-y-1 text-sm">
                            ...
                            {mangDanhMuc.map((danhMuc, index) => {
                                ...
                                return <li key={index} className="dark:bg-coolGray-800 dark:text-coolGray-50" onClick={() => {
                                    setMaDanhMuc(danhMuc.maDanhMuc);
                                }}>
                                    <a rel="noopener noreferrer" href="#" className={_.join(classNameActive, ' ')}>
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" className="w-5 h-5 fill-current dark:text-coolGray-400">
                            ...
                            {_.filter(data, item => item.maDanhMuc === maDanhMuc).map((item, index) => {
                                return <div key={index} className="p-4 lg:w-1/3">
                                    <div className="h-full bg-gray-100 bg-opacity-75 px-8 pt-16 pb-24 rounded-lg overflow-hidden text-center relative">
                                        <h2 className="tracking-widest text-xs title-font font-medium text-gray-400 mb-1">{item.tenDanhMuc}</h2>
                                        <h1 className="title-font sm:text-2xl text-xl font-medium text-gray-900 mb-3">{item.tenMonAn}</h1>
                                        <div className="leading-relaxed mb-3"><img src={item.hinhAnh} alt={item.tenMonAn} /></div>
                                        ...
                                        <div className="text-right mt-2 leading-none flex justify-end absolute bottom-0 left-0 w-full py-4">
